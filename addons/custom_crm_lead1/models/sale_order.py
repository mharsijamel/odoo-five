...
class SaleOrder(models.Model):
    ...
    def onchange_contract_id_signed_id(self):
        for order in self:
            if order.contract_signed or order.contract_id_signed:
                order.sudo().write({'installation_state': 'contractual_commitment'})

    @api.onchange('commitment_date')
    def onchange_commitment_date_id(self):
        for order in self:
            if order.commitment_date:
                order.sudo().write({'installation_state': 'planning'})

    @api.onchange('state')
    def onchange_state_signed_id(self):
        for order in self:
            if order.state == 'sale':
                order.sudo().write({'installation_state': 'pre_installation'})
        ...
        for order in self:
            if order.lead_reception_date:
                if order.opportunity_id:
                    order.opportunity_id.sudo().write({'lead_reception_date': order.lead_reception_date})
    ...
    def onchange_lead_preparation_date(self):
        for order in self:
            if order.lead_preparation_date:
                if order.opportunity_id:
                    order.opportunity_id.sudo().write({'lead_preparation_date': order.lead_preparation_date})
    ...
    def onchange_lead_filling_date(self):
        for order in self:
            if order.lead_filling_date:
                if order.opportunity_id:
                    order.opportunity_id.sudo().write({'lead_filling_date': order.lead_filling_date})
    ...
    def onchange_lead_approbation_date(self):
        for order in self:
            if order.lead_approbation_date:
                if order.opportunity_id:
                    order.opportunity_id.sudo().write({'lead_approbation_date': order.lead_approbation_date})
    ...
    def onchange_poip_fees(self):
        for order in self:
            if order.poip_fees:
                if order.opportunity_id:
                    order.opportunity_id.sudo().write({'poip_fees': order.poip_fees})
    ...
    def onchange_steg_credit(self):
        for order in self:
            if order.steg_credit:
                if order.opportunity_id:
                    order.opportunity_id.sudo().write({'steg_credit': order.steg_credit})
    ...
    def onchange_sub_anme(self):
        for order in self:
            if order.sub_anme:
                if order.opportunity_id:
                    order.opportunity_id.sudo().write({'sub_anme': order.sub_anme})
    ...
    def onchange_filing_date(self):
        for order in self:
            if order.filing_date:
                if order.opportunity_id:
                    order.opportunity_id.sudo().write({'filing_date': order.filing_date})
    ...
    def onchange_approbation_date(self):
        for order in self:
            if order.approbation_date:
                if order.opportunity_id:
                    order.opportunity_id.sudo().write({'approbation_date': order.approbation_date})
