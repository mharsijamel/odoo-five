...
class CrmLead(models.Model):
    ...
    def onchange_attachment_contract(self):
        for lead in self:
            if lead.steg_credit:
                for order in lead.order_ids.search([], limit=1):
                    SaleOrder = self.env['sale.order'].search([('name', '=', order.name)], limit=1)
                    if SaleOrder:
                        SaleOrder.sudo().write({
            ...
            if lead.lead_approbation_date:
                ...
                if lead.order_ids:
                    for order in lead.order_ids:
                        ...
                        SaleOrder = self.env['sale.order'].search([('name', '=', order.name)])
    ...
    def onchange_approbation_date(self):
        for lead in self:
            if lead.approbation_date:
                ...
                lead.sudo().write({'installation_state': 'internal_order'})
                if lead.order_ids:
                    for order in lead.order_ids:
                        if not lead.poip_attachement or not lead.discharge_attachement:
                            raise UserError(
    ...
    def onchange_contract_id_signed_id(self):
        for lead in self:
            if lead.contract_signed:
                lead.sudo().write({'installation_state': 'contractual_commitment'})
                if lead.order_ids:
                    for order in lead.order_ids:
                        order.sudo().write({
        ...
        for opportunity in self:
            if opportunity.partner_id:
                if not opportunity.partner_id.is_customer:
                    opportunity.partner_id.sudo().write({
                        'is_customer': True,
                        'customer_rank': 1
                    })
