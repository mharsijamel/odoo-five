...
class gen_journal_entry(models.TransientModel):
    ...
    def find_account_id(self, account_code):
        if account_code:
            ...
            print("Code", account_code.split('.'))
            print("Code", account_ids.code)
            if account_ids:
                account_id = account_ids[0]
                return account_id
            else:
                raise Warning(_('"%s" Wrong Account Code') % (str(account_code)))
    ...
    def find_partner(self, partner_name):
        partner_ids = self.env['res.partner'].search([('name', 'ilike', partner_name)])
        if partner_ids:
    ...
    def create_import_move_lines(self, values):
        ...
        if values.get('account_code'):
            account_code = str(values.get('account_code'))
        ...
        if values.get('debit') != '':
            print("debit", str(values.get('debit')))
        ...
        if values.get('credit') != '':
            print("credit", str(values.get('credit')))
        ...
        if self.import_option == 'csv':
                ...
                if row_no <= 0:
                    ...
                    if line[7] != '' and line[0] != '':
                        ...
                        values = {'date': main_date,
                                  ...
                                  'account_code': str(line[6]),
                                  'date_maturity': date,
                                  'debit': line[8],
                                  'credit': line[9],
                                  'amount_currency': line[10],
                                  'currency': line[11],
                                  'num_piece': str(line[12])
                                  }
                        data.append(values)
                    else:
            ...
            for key in data1.keys():
                ...
                for val in values:
                    ...
                    if val.get('journal'):
                        journal_search = self.env['account.journal'].sudo().search(
                            [('name', 'ilike', val.get('journal')), ('company_id', '=', self.company_id.id)])
                        if journal_search:
                            # date = datetime.strptime(val.get('date'), "%Y-%m-%d")
                            print(type(val.get('num_piece')))
                            num_piece = val.get('num_piece').split(".")
                            num_piece = num_piece[0]
                            ref = val.get('ref').split('.')
                            ref = ref[0]
                            ...
                            if move1:
                                ...
                                move = move_obj.create({
                                    ...
                                    'ref': ref or False,
                                    'journal_id': journal_search.id,
                                    'num_piece': num_piece or False,
                                })
                                # if values.get('num_piece') == '':
                                #     values.update({'num_piece': ''})

                        else:
                            raise Warning(_('Please Define Journal which are already in system.'))
                    del res['journal'], res['partner'], res['account_code'], res['currency'], res['num_piece']
                    # print("RES", res)
                    lines.append((0, 0, res))
