...
class FleetVehicle(models.Model):
    ...
    driver_id = fields.Many2one('res.users', 'Driver', tracking=True, help='Driver address of the vehicle', copy=False)
    future_driver_id = fields.Many2one('res.users', 'Future Driver', tracking=True, help='Next Driver Address of the vehicle', copy=False, domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    ...
    def create(self, vals):
        ...
        if 'future_driver_id' in vals and vals['future_driver_id']:
            ...
            if not state_waiting_list or state_waiting_list.id not in states:
                future_driver = self.env['res.users'].browse(vals['future_driver_id'])
